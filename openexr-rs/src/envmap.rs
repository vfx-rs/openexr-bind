...
fn test_data_window_for_face() {
    let dw = [0, 0, 399, 299];
    let fdw = cube_map::data_window_for_face(crate::CubeMapFace::PosY, &dw);
...
fn test_face_and_position_in_face_from_direction() {
    ...
    let (face, position_in_face): (crate::CubeMapFace, [f32; 2]) =
        cube_map::face_and_position_in_face_from_direction(&dir, &dw);

    assert_eq!(face, crate::CubeMapFace::PosY);
